...
struct student getstudent(int sno){
        int i=sno;
        struct student s;
        int fd=open("home/Desktop/MiniProject/data/Student.data",O_RDONLY,0744);
        ...
        read(fd,&s,sizeof(struct student)); //copy details of current student to struct s
...
bool checkNormalUser(struct student s){
        int i=currUser.userID;
        int fd=open("/home/ubuntu/MT2022142/Data/Student.data",O_RDONLY,0744);
        struct student temp;
        struct flock lock;
        lock.l_type = F_RDLCK;
        lock.l_whence=SEEK_SET;
        ...
        if(!strcmp(temp.password,s.password) && !strcmp(temp.status,"ACTIVE"))   
        	result=true;
        else                                            
        	result=false;
